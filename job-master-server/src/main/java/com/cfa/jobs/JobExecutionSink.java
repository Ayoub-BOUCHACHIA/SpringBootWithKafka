...
public class JobExecutionSink {
    ...
    @ServiceActivator(inputChannel = "errorChannelExo")
    public void errorChannelExo(final ErrorMessage errorMessage) {
        final Message<?> locFailedMessage = ((KafkaSendFailureException) errorMessage.getPayload()).getFailedMessage();
        assert locFailedMessage != null;
        final MessageHeaders locHeaders = locFailedMessage.getHeaders();
        log.info("errorChannelExo, MESSAGE NOT SEND : {} ", locHeaders);
    }

    @SneakyThrows
    @StreamListener(target = Sink.INPUT, condition = "headers['custom_infoExo']=='end'")
    public void receiveResultExo(final Message<String> message) {
        log.info("[Server Exo END] received result {} ", message.getPayload());
    }
